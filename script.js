...
function adicionarAoCarrinho(id) {
 ...
 if (produto){
  ...
  if (produtoNoCarrinho){
    produtoNoCarrinho.quantidade +=1; // quantidade foi declarad agora
  } else{
    colecaoCarrinho.push({...produto, quantidade: 1});
  }
 }
 renderizarCarrinho();
 asideCart.classList.add("active"); //o asideCart (declarado na linha 2)
}
...
function renderizarProdutosPrincipal(produtos) {
  ...
  const totalprodutos = document.querySelector('#toalProdutos');
